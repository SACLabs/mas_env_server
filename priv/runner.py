...
venv_path = os.path.join(venv_path, "envs/fairs")
...
def generate_code_project(code_tree, code_str):
    project_name = code_tree.split(' ')[0]
    ...
    for file_path, file_content in code_str.items():
        file_dir = f"/tmp/{os.path.dirname(file_path)}"
        file_name = os.path.basename(file_path)
        os.makedirs(file_dir, exist_ok=True)
        with open(os.path.join(file_dir, file_name), "w") as f:
            f.write(file_content)
    # 写入运行脚本
    with open(f"{project_path}/run.sh", "w") as file:
        file.write("python -m cProfile -o output.prof main.py")
    return project_path


def run_ci(code_tree, code_str):
    source_code_path = generate_code_project(code_tree, code_str)
...
if __name__ == "__main__":
    import sys
    import json
    code_tree = sys.argv[1]
    code_str = json.loads(sys.argv[2])
    pytest_result, performance_result = run_ci(code_tree, code_str)
    print(json.dumps({"pytest_result": pytest_result, "performance_result": performance_result}))
